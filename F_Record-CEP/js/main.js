...
function startOrStopRecord() {
  ...
  if (btn.classList.contains("topcoat-button--cta")) {
    ...
    csInterface.evalScript(
      `$._ext.sendToGenerator('${JSON.stringify({
        from: "panel",
        action: "start-record",
      })}')`
    );
    ...
    if (document.getElementById("imageCount").innerHTML == "0") {
      ...
      btn.innerHTML = "Continue / Resume Recording";
    }
...
function updateSavePath() {
  ...
  csInterface.evalScript(
    `$._ext.sendToGenerator('${JSON.stringify({
      ...
      value: encodeURIComponent(localStorage["savePath"]),
    })}')`
  );
  loadImageCount();
}
...
function updateMinRecordInterval() {
  ...
  localStorage["minRecordInterval"] =
    document.getElementById("minRecordInterval").value;
  csInterface.evalScript(
    `$._ext.sendToGenerator('${JSON.stringify({
      from: "panel",
      action: "update-min-record-interval",
      value: parseFloat(localStorage["minRecordInterval"]),
    })}')`
  );
}
...
function loadImageCount() {
  ...
  if (
    document
      ...
      .classList.contains("topcoat-button--cta")
  ) {
    if (imageFiles.length == 0) {
      document.getElementById("start-btn").innerHTML = "Start Recording";
    } else {
      document.getElementById("start-btn").innerHTML = "Continue Recording";
    }
  }
}
...
async function outputVideo() {
  ...
  try {
    ...
    alert("This path could not be found.");
    return;
  }
  const save_path = localStorage["savePath"];
  ...
  if (localStorage["videoDuration"] == "") {
    alert("Please fill in the recording duration.");
    return;
  }
  if (imageFiles.length == 0) {
    alert("There’s nothing here — I can’t draw anything for you!");
    return;
  }
  if (imageFiles.length == 917) {
    //magic number
    if (Math.random() < 0.5) {
      alert("Take care.~");
    }
  ...
  const lastImageData = jpeg.decode(
    fs.readFileSync(
      path.join(imageDirectory, imageFiles[imageFiles.length - 1])
    )
  );
  ...
  try {
    ...
    async function output_0(p) {
      ...
      for (let i = 0; i < q - p; i++) {
        complexFilter += `[${
          i + 1
        }:v]scale=w='iw*min(${videoWidth}/iw,${videoHeight}/ih)':h='ih*min(${videoWidth}/iw,${videoHeight}/ih)'[img${
          i + 1
        }];`;
      }
      for (let i = 0; i < q - p; i++) {
        ...
        if (i === 0) {
          complexFilter += `[videobase][img${
            i + 1
          }]overlay='main_w/2-overlay_w/2':'if(gte(t, ${t1}), if(gte(t, ${t2}),main_h,main_h/2-overlay_h/2), main_h)'${
            i === q - p - 1 ? "" : `[a${i}];`
          }`;
        } else {
          complexFilter += `[a${i - 1}][img${
            i + 1
          }]overlay='main_w/2-overlay_w/2':'if(gte(t, ${t1}), if(gte(t, ${t2}),main_h,main_h/2-overlay_h/2), main_h)'${
            i === q - p - 1 ? "" : `[a${i}];`
          }`;
    ...
    async function output_3(p) {
      ...
      if (imageFiles.length < 1000) {
        alert(`Recording generated successfully.${outputVideo}`);
      } else {
        alert(`You just made something Absolute Masterpiece!${outputVideo}`);
      }
    }
    await output_0(0);
    tmpFiles = fs
      .readdirSync(tmpDirectory0)
      .filter((file) => file.endsWith(".mp4"));
    await output_1(0);
    tmpFiles = fs
      .readdirSync(tmpDirectory1)
      .filter((file) => file.endsWith(".mp4"));
    btn.innerHTML = 93 + "%";
    await output_2(0);
    tmpFiles = fs
      .readdirSync(tmpDirectory2)
      .filter((file) => file.endsWith(".mp4"));
    btn.innerHTML = 96 + "%";
    await output_3(0);
  } catch (error) {
    ...
    btn.classList.add("topcoat-button--cta");
    btn.innerHTML = "Export Recording";
...
function clearProcessImages() {
  var result = confirm(
    "Are you sure you want to delete the current recording?Make sure you’re happy with the exported video before doing so."
  );
  if (!result) {
    return;
  }
  ...
  try {
    ...
    alert("The specified path could not be found.”");
    return;
  }
  const imageDirectory = path.join(save_path, "process_images");
  try {
    ...
    alert("There’s nothing here — you cleared for nothing.");
    return;
  }
  ...
  btn.style.pointerEvents = "auto";
  btn.classList.add("topcoat-button--cta");
  btn.innerHTML = "Process clearing";
}
...
window.addEventListener("load", function () {
  ...
  csInterface.addEventListener(
    "com.generator.plugin.demo",
    handleEventFromGenerator
  );
